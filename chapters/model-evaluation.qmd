...
A good model makes predictions with a **short distance** to the truth.
...
At the end of the training process, the trained model (like [K-Nearest Neighbours](neighbours.qmd){target=_blank}) can be used to generate predictions on the test data. For this portion of unseen data, we have both model predictions and the true labels.
...
![Surface area distribution for both Flats and Houses](/epub_images/model-evaluation/whole_dataset.jpg)
:::
...
::: {.content-visible when-format="epub"}
![Surface area distributions in the training and test sets](/epub_images/model-evaluation/two_sets.jpg)
:::
...
::: {.content-visible when-format="epub"}
![Random split vs time-based split](/epub_images/model-evaluation/time_based_split.jpg)
:::
